...
public static class SeedData
{
    public static async Task SeedAsync(ApplicationDbContext context)
    {
        ...
        // Attendances - Now creates records using actual course instance days
        var attendances = new List<Attendance>();
        var random = new Random();

        foreach (var enrollment in enrollments)
        {
            var courseInstance = courseInstances.First(ci => ci.Id == enrollment.CourseInstanceId);

            for (int week = 1; week <= 15; week++)
            {
                // Create attendance for Day1
                var statusValue1 = random.Next(0, 100);
                var status1 = statusValue1 >= 90 ? "Absent" : "Present";
                ...
                // Create attendance for Day2 if it exists
                if (courseInstance.Day2.HasValue)
                {
                    var statusValue2 = random.Next(0, 100);
                    var status2 = statusValue2 >= 90 ? "Absent" : "Present";

                    attendances.Add(new Attendance
                    {
                        EnrollmentId = enrollment.Id,
                        Week = week,
                        Day = courseInstance.Day2,
                        Status = status2,
                        CreatedAt = DateTime.UtcNow
                    });
                }
